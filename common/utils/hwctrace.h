...
class TraceFunc {
 public:
  TraceFunc(std::string func_name) {
    ...
    ILOG("Calling ----- %s", func_name_.c_str());
  ...
  ~TraceFunc() {
    ...
    ILOG("Leaving --- %s", func_name_.c_str());
  }
...
#define DUMPTRACE(fmt, ...) ITRACE(fmt, ##__VA_ARGS__)
#else
...
#define IPAGEFLIPEVENTTRACE(fmt, ...) ITRACE(fmt, ##__VA_ARGS__)
#else
#define IPAGEFLIPEVENTTRACE(fmt, ...) ((void)0)
#endif

#ifdef ENABLE_DISPLAY_MANAGER_TRACING
#define IDISPLAYMANAGERTRACE(fmt, ...) ITRACE(fmt, ##__VA_ARGS__)
#else
#define IDISPLAYMANAGERTRACE(fmt, ...) ((void)0)
#endif

#ifdef ENABLE_HOT_PLUG_EVENT_TRACING
#define IHOTPLUGEVENTTRACE(fmt, ...) ITRACE(fmt, ##__VA_ARGS__)
#else
#define IHOTPLUGEVENTTRACE(fmt, ...) ((void)0)
#endif

#ifdef COMPOSITOR_TRACING
#define ICOMPOSITORTRACE(fmt, ...) ITRACE(fmt, ##__VA_ARGS__)
#else
